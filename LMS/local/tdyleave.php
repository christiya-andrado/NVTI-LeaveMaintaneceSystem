...
<html lang="en">
  ...
  <body>
    ...
    <main class="mt-5 pt-3">
      <div class="container">
      <br>
      <h2> Today Leave Details Of Centres </h2>
      <hr>
        <?php
            include "connection.php";
            // include "tdylv.php";
...
<?php
            ...
            // $sql = "SELECT e.CenterLocation, COUNT(DISTINCT l.Empid), v.Total_Employees
            // FROM leave_details AS l LEFT JOIN employee AS e ON l.Empid=e.Empid INNER JOIN vwcentre_info AS v ON e.CenterLocation=v.CenterLocation
            // WHERE l.Status=1 AND CURDATE() BETWEEN l.FromDate AND l.ToDate GROUP BY e.CenterLocation";

            $sql="SELECT * FROM vwcentre_leaveinfo";
            $res=mysqli_query($con,$sql);

            while($row=mysqli_fetch_assoc($res))
            {
              $centre=$row['CenterLocation'];
              $lvemp=$row['Employees_On_Leave'];
