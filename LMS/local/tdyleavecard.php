...
<html lang="en">
  <head>
    ...
    <title>Today Leave Details</title>
    <style>
      #count{
        border-radius:50%;
        position: relative;
    ...
    <hr class="dropdown-divider bg-light my-lg-0" />
      ...
      <div class="offcanvas-body p-0">
        <nav class="navbar-dark">
          ...
          border-radius: 5px; margin-left: 15px;" name="b2" ><i class="bi bi-box-arrow-left" href="logout.php"></i> | Logout</a>
    ...
    <main class="mt-5 pt-3">
      ...
      <hr>
        <?php
          ...
          //       <th>#</th>
          //       <th>Centre</th>                
          //       <th>No. Of Employees In Leave</th> 
          ...
          //   </thead>';
            ...
            $sql = "SELECT e.CenterLocation, COUNT(DISTINCT l.Empid) FROM leave_details AS l LEFT JOIN employee AS e ON l.Empid=e.Empid WHERE l.Status=1 AND CURDATE() BETWEEN l.FromDate AND l.ToDate GROUP BY e.CenterLocation";

            $res=mysqli_query($con,$sql);

            while($row=mysqli_fetch_assoc($res))
            {
              $centre=$row['CenterLocation'];
              $lvemp=$row['COUNT(DISTINCT l.Empid)'];
?>
              ...
              <!-- <div class="row">
                    <div class="col-sm-6"> -->
                        <div class="card bg-secondary" >
                            <div class="card-body">
                                <?php echo $s_no++; ?>
                                <h4 class="card-title"> Centre : <?php echo $centre; ?></h4> <hr>
                                <p class="card-text"> No. Of Employees In Leave = <?php echo $lvemp; } ?> </p> <hr> <br>
                                <?php
            
                                 $sql1="SELECT COUNT(`Empid`) FROM `employee` WHERE CenterLocation IN (SELECT e.CenterLocation FROM leave_details AS l LEFT JOIN employee AS e ON l.Empid=e.Empid WHERE l.Status=1 AND CURDATE() BETWEEN l.FromDate AND l.ToDate GROUP BY e.CenterLocation) GROUP BY CenterLocation";
            
                                 $res1=mysqli_query($con,$sql1);
                     
                                 while($row1=mysqli_fetch_assoc($res1))
                                 { 
                                    $totemp=$row1['COUNT(`Empid`)'];
                                    ?>
                                  <p class="card-text"> Total Employees In <?php echo $centre; ?> = <?php echo $totemp ?> </p> <hr>
                          <?php }
                                  ?>
                              </div>
                        </div>
